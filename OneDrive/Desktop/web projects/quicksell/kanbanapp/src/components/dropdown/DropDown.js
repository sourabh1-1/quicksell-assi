...
export default function DropDown({view,setView}) {
    ...
    function handleDisplayToggle(e){
        ...
        if(e.target.value !== undefined){
            setView({...view,groupBy:e.target.value});
            localStorage.setItem("view",JSON.stringify({...view,groupBy:e.target.value}));
        }
    }
    function handleOrderingValue(e){
        settoggleFilter(!toggleFilter);
        if(e.target.value !== undefined){
            setView({...view,sortBy:e.target.value});
            localStorage.setItem("view",JSON.stringify({...view,orderBy:e.target.value}));
        }
    }
                    ...
                    <div className={toggleFilter ? "nav-disp-dropdown nav-disp-dropdown-show" : "nav-disp-dropdown"}>
                        <div className="nav-disp-filters">
                            ...
                            <div className="nav-dropdown-selector">
                                <select value={view.groupBy} onChange={handleDisplayToggle} className='nav-selector' name="grouping" id="">
                        ...
                        <div className="nav-disp-filters">
                            ...
                            <div className="nav-dropdown-selector">
                                <select value={view.orderBy} onChange={handleOrderingValue} className='nav-selector' name="grouping" id="">
                                    <option value="priority">Priority</option>
